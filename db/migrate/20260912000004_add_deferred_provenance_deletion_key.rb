...
class AddDeferredProvenanceDeletionKey < ActiveRecord::Migration[7.1]
  ...
  def down
    ...
    swap_to_event_index
    remove_column TABLE, :deletion_key if column_exists?(TABLE, :deletion_key)
  end
  ...
  def swap_to_event_index
    remove_index TABLE, name: NEW_INDEX if index_name_exists?(TABLE, NEW_INDEX)
    return if index_name_exists?(TABLE, OLD_INDEX)

    add_index TABLE, %i[conversation_id prior_assignee_id event], unique: true, name: OLD_INDEX
  end
end
