...
describe Messages::Facebook::MessageBuilder do
  ...
  describe '#perform' do
    ...
    context 'when Messenger referral data is present' do
      before do
        allow(Koala::Facebook::API).to receive(:new).and_return(fb_object)
        allow(fb_object).to receive(:get_object).and_return({ first_name: 'Jane', last_name: 'Dae',
                                                              profile_pic: 'https://chatwoot-assets.local/sample.png' }.with_indifferent_access)
      end

      let(:referral_message_object) do
        ads_context_data = { ad_title: 'Messenger Ad', photo_url: 'https://example.com/msg.jpg',
                             video_url: 'https://example.com/msg-video.jpg', post_id: 'post-1', product_id: 'product-1' }
        referral = { ref: 'custom-ref', ad_id: 'msg-ad-1', source: 'ADS', type: 'OPEN_THREAD', ads_context_data: ads_context_data }
        { messaging: { sender: { id: '3383290475046708' }, recipient: { id: facebook_channel.page_id },
                       message: { mid: 'm_referral_1', text: 'messenger ad click', referral: referral } } }.to_json
      end
      ...
      it 'stores normalized ads_referral on the first inbound message' do
        ...
        expect(referral).to include('channel' => 'messenger', 'source_id' => 'msg-ad-1', 'source_type' => 'ad',
                                    'headline' => 'Messenger Ad', 'image_url' => 'https://example.com/msg.jpg',
                                    'video_url' => 'https://example.com/msg-video.jpg', 'ref' => 'custom-ref',
                                    'post_id' => 'post-1', 'product_id' => 'product-1')
      end
