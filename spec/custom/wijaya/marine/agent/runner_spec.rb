...
RSpec.describe Marine::Agent::Runner do
  ...
  describe 'independence from Captain premium gates' do
    it 'does not reference any Captain runtime dependency in the source' do
      source = File.read(Rails.root.join('custom/wijaya/marine/app/services/marine/agent/runner.rb'))
      expect(source).not_to match(/Captain::|ChatwootHub|hub\.2\.chatwoot\.com|pricing_plan|CAPTAIN_CLOUD_PLAN_LIMITS|FEATURE_FLAGS\.CAPTAIN|captain_integration/)
    end
  end
end
