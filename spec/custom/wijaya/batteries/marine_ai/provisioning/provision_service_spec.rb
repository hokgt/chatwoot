...
RSpec.describe Marine::Provisioning::ProvisionService do
  ...
  describe '#call happy path' do
    it 'creates owner, login and database, then persists active non-secret state' do
      ...
      owner_role_ddl = /CREATE ROLE "marine_erp_owner" NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS/
      expect(conn).to have_received(:exec).with(owner_role_ddl)
      ...
      login_role_ddl = /CREATE ROLE "marine_app" NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS PASSWORD/
      expect(conn).to have_received(:exec).with(login_role_ddl)
