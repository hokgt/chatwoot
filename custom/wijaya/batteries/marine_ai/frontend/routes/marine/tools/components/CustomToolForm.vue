...
import MarineCustomToolsAPI from '@wijaya/marine_ai/frontend/api/customTools';
...
const authTypeOptions = computed(() => [
  { value: 'none', label: t('MARINE_AI.CUSTOM_TOOLS.FORM.AUTH_TYPES.NONE') },
  {
    value: 'bearer',
    label: t('MARINE_AI.CUSTOM_TOOLS.FORM.AUTH_TYPES.BEARER'),
  },
  { value: 'basic', label: t('MARINE_AI.CUSTOM_TOOLS.FORM.AUTH_TYPES.BASIC') },
  {
    value: 'api_key',
    label: t('MARINE_AI.CUSTOM_TOOLS.FORM.AUTH_TYPES.API_KEY'),
  },
]);
...
const getErrorMessage = (field, errorKey) => {
  ...
  if (failedRule === 'maxLength') {
    return t(`MARINE_AI.CUSTOM_TOOLS.FORM.${errorKey}.MAX_LENGTH_ERROR`, {
      max: MAX_TOOL_NAME_LENGTH,
    });
  }
  return t(`MARINE_AI.CUSTOM_TOOLS.FORM.${errorKey}.ERROR`);
};
...
const handleTest = async () => {
  ...
  try {
    const { data } = await MarineCustomToolsAPI.test(state);
    const isOk = data.status >= 200 && data.status < 300;
    testResult.value = { success: isOk, status: data.status };
  } catch (e) {
    const message =
      e.response?.data?.error || t('MARINE_AI.CUSTOM_TOOLS.TEST.ERROR');
...
<template>
  ...
  >
    <Input
      v-model="state.title"
      :label="t('MARINE_AI.CUSTOM_TOOLS.FORM.TITLE.LABEL')"
      :placeholder="t('MARINE_AI.CUSTOM_TOOLS.FORM.TITLE.PLACEHOLDER')"
    ...
    <TextArea
      ...
      :label="t('MARINE_AI.CUSTOM_TOOLS.FORM.DESCRIPTION.LABEL')"
      :placeholder="t('MARINE_AI.CUSTOM_TOOLS.FORM.DESCRIPTION.PLACEHOLDER')"
      :rows="2"
    />
    ...
    <div class="flex gap-2">
      <div class="flex flex-col gap-1 w-28">
        <label class="mb-0.5 text-sm font-medium text-n-slate-12">
          {{ t('MARINE_AI.CUSTOM_TOOLS.FORM.HTTP_METHOD.LABEL') }}
        </label>
        <ComboBox
      ...
      <Input
        ...
        :label="t('MARINE_AI.CUSTOM_TOOLS.FORM.ENDPOINT_URL.LABEL')"
        :placeholder="t('MARINE_AI.CUSTOM_TOOLS.FORM.ENDPOINT_URL.PLACEHOLDER')"
    ...
    <div class="flex flex-col gap-1">
      <label class="mb-0.5 text-sm font-medium text-n-slate-12">
        {{ t('MARINE_AI.CUSTOM_TOOLS.FORM.AUTH_TYPE.LABEL') }}
      </label>
      <ComboBox
    ...
    <div class="flex flex-col gap-2">
      <label class="text-sm font-medium text-n-slate-12">
        {{ t('MARINE_AI.CUSTOM_TOOLS.FORM.PARAMETERS.LABEL') }}
      </label>
      <p class="text-xs text-n-slate-11 -mt-1">
        {{ t('MARINE_AI.CUSTOM_TOOLS.FORM.PARAMETERS.HELP_TEXT') }}
      </p>
      ...
      <Button
        ...
        blue
        icon="i-lucide-plus"
        :label="t('MARINE_AI.CUSTOM_TOOLS.FORM.ADD_PARAMETER')"
        @click="addParam"
      />
    </div>
    ...
    <TextArea
      ...
      :label="t('MARINE_AI.CUSTOM_TOOLS.FORM.REQUEST_TEMPLATE.LABEL')"
      :placeholder="
        t('MARINE_AI.CUSTOM_TOOLS.FORM.REQUEST_TEMPLATE.PLACEHOLDER')
      "
      :rows="4"
    ...
    <TextArea
      ...
      :label="t('MARINE_AI.CUSTOM_TOOLS.FORM.RESPONSE_TEMPLATE.LABEL')"
      :placeholder="
        t('MARINE_AI.CUSTOM_TOOLS.FORM.RESPONSE_TEMPLATE.PLACEHOLDER')
      "
      :rows="4"
      class="[&_textarea]:font-mono"
    />
    ...
    <div class="flex flex-col gap-2">
      ...
      >
        ...
        {{
          testResult.status
            ? t('MARINE_AI.CUSTOM_TOOLS.TEST.SUCCESS', {
    ...
    <div class="flex gap-3 justify-between items-center w-full">
      <Button
        ...
        variant="faded"
        color="slate"
        :label="t('MARINE_AI.CUSTOM_TOOLS.FORM.CANCEL')"
      ...
      <Button
        ...
        :label="
          t(
            mode === 'edit'
              ? 'MARINE_AI.CUSTOM_TOOLS.FORM.EDIT'
              : 'MARINE_AI.CUSTOM_TOOLS.FORM.CREATE'
          )
        "
        class="w-full"
