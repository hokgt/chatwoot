# Translates an inbound customer query into the assistant/knowledge base
# language before retrieval. The LLM is only invoked when a translation is
# actually needed (source language is known and differs from the target) and
# Marine LLM is configured. It never raises: callers always receive a JSON-safe
# hash so the retrieval pipeline can branch without rescuing.
#
# Returns:
#   { ok:, text:, original_text:, source_language:, target_language:,
#     translated:, error:, skipped_reason? }
class Marine::Llm::TranslateQueryService
  DEFAULT_TARGET = 'en'.freeze

  def initialize(text:, target_language: nil, account: nil)
    @text = text.to_s
    @target_language = normalize(target_language) || DEFAULT_TARGET
    @account = account
  end

  def call
    detection = Marine::Llm::LanguageDetector.new(@text).detect
    source = normalize(detection[:language])

    return skip(source, 'blank_input') if @text.strip.blank?
    return skip(source, 'unknown_source_language') if source.blank?
