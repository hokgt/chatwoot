# == Schema Information
#
# Table name: marine_custom_tools
#
...
class Marine::CustomTool < ApplicationRecord
  ...
  include Marine::Toolable
  include Marine::SafeEndpointValidatable

  self.table_name = 'marine_custom_tools'

  NAME_PREFIX = 'custom'.freeze
  NAME_SEPARATOR = '_'.freeze
  # OpenAI enforces a 64-char limit on function names. The slug is used verbatim
  # as the tool name in LLM requests, so it must fit within this limit.
  MAX_SLUG_LENGTH = 64
  ...
  def ensure_within_limit
    ...
    return if account.marine_custom_tools.count < MAX_PER_ACCOUNT

    raise LimitExceededError, I18n.t('marine.custom_tool.limit_exceeded', limit: MAX_PER_ACCOUNT)
  end
  ...
  def find_unique_slug(base_slug)
    ...
    raise ActiveRecord::RecordNotUnique, I18n.t('marine.custom_tool.slug_generation_failed')
  end
