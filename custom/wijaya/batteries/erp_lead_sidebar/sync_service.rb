      ...
      class SyncService
        ...
        def sync_existing(name, payload)
          ...
          if response.is_a?(Net::HTTPNotFound)
            raise SyncError, "Stored ERP Lead '#{name}' was not found in ERPNext; relink required."
          end

          unless response.is_a?(Net::HTTPSuccess)
            raise SyncError, "ERPNext sync failed: #{error_message(body, response)}"
          end
        ...
        def find_existing_lead_by_phone(payload)
          ...
          phones.each do |phone|
            %w[whatsapp_no mobile_no].each do |field|
