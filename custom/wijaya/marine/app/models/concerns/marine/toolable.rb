# Mirrors Concerns::Toolable (Captain) but kept under the Marine namespace and
# wired to Marine::Tools::HttpTool. Builds a per-tool subclass so the Marine LLM
# can later expose the tool, and provides the request/auth/metadata/response
# helpers shared with the HTTP execution wrapper.
module Marine::Toolable
  extend ActiveSupport::Concern

  # Isolated namespace for user-defined custom tool classes, keeping them separate
  # from built-in classes in Marine::Tools (e.g., HttpTool).
  module CustomTools; end

  def tool(assistant, base_class: Marine::Tools::HttpTool, **)
    ...
    tool_class = Class.new(base_class) do
      ...
      # that the default normalization would produce.
    ...
    # Anonymous classes return nil for #name, which causes empty function name
